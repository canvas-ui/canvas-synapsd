...
## Session support — container LANDED; UI/transport plan MOVED

`src/session/QuerySession.js` (via `db.openSession`) implements the full container: modes
`frozen|live`, emit `delta|ids|page`, `add/remove/patch/clear` + `set()` replace-mode upsert,
the `ids` spec bucket, `count/ids/materialize`, precise `membership.changed` invalidation with
coarse re-resolve, debounced recompute, `serialize()/rehydrate()`. Tests:
`tests/query-session.test.js`.

The session TRANSPORT + delta-driven UI plan (socket.io session RPC in canvas-server, webui
`useQuerySession` hook, sensord consumer surface) lives in the canvas monorepo now:
`canvas/TODO.md` — it is canvas-server/webui work, not engine work.

Synapsd-local leftover:

- [ ] session-wide `andNot(internal/gc/deleted)` in `QuerySession#combine()` — an id-set cue
      has no collection keys, so deletes never dirty it; excluding tombstones session-wide
      fixes that generally (parked from the id-set operand work).
