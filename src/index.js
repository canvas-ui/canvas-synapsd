...
class SynapsD extends EventEmitter {
    ...
    // Serialize document writes from preparation through publication so ID
    // allocation and dedup cannot join another operation's suspended transaction.
    // Each transaction buffers membership notifications and events. Direct index
    // writes remain in LMDB; their mutable caches are restored on rollback.
    ...
    async put(document, spec = {}) {
        return this.#withWriteLock(() => this.#put(document, spec));
    }
    ...
    async link(idOrIds, spec = {}) {
        return this.#withWriteLock(() => this.#link(idOrIds, spec));
    }
    ...
    async relate(fromId, predicate, toId, options = {}) {
        return this.#withWriteLock(() => this.#relate(fromId, predicate, toId, options));
    }
    ...
    async unrelate(fromId, predicate, toId) {
        return this.#withWriteLock(() => this.#unrelate(fromId, predicate, toId));
    }
    ...
    async assertRelation(docId, predicate, toId) {
        return this.#withWriteLock(() => this.#writeAssertedRelation(docId, predicate, toId, 'assert'));
    }
    ...
    async retractRelation(docId, predicate, toId) {
        return this.#withWriteLock(() => this.#writeAssertedRelation(docId, predicate, toId, 'retract'));
    }
    ...
    async unlink(idOrIds, spec = {}) {
        return this.#withWriteLock(() => this.#unlink(idOrIds, spec));
    }
    ...
    async delete(id, options = {}) {
        return this.#withWriteLock(() => this.#delete(id, options));
    }
    ...
    async putMany(documents, spec = {}) {
        return this.#withWriteLock(() => this.#putMany(documents, spec));
    }
        ...
        try {
            await this.#withDeferredMembership(async () => {
                ...
                for (const { parsed, existing, isUpdate, previous, docFeatures } of prepared) {
                    ...
                    if (existing && previous) {await this.#removeDocumentTimelines(parsed.id, previous, parsed);}
                    await this.#indexDocumentTimelines(parsed.id, parsed);
                    await this.#indexDocumentGeo(parsed.id, parsed);
                    this.#syncDocumentRelations(parsed.id, previous?.relations, documentRelations(parsed));
                    await this.#indexDocument(parsed.id, contextSpec, directorySpec, docFeatures);
                    if (existing) {
                        await this.#removeStaleLocationMembership(parsed.id, { locations: previous?.locations, orphanedAt: previous?.orphanedAt }, parsed, docFeatures);
                    }
    ...
    async putManyDirectoryPaths(items, treeName, featureArray = [], options = {}) {
        return this.#withWriteLock(() => this.#putManyDirectoryPaths(items, treeName, featureArray, options));
    }
    ...
    async linkMany(ids, spec = {}) {
        return this.#withWriteLock(() => this.#linkMany(ids, spec));
    }
    ...
    async unlinkMany(ids, spec = {}) {
        return this.#withWriteLock(() => this.#unlinkMany(ids, spec));
    }
    ...
    async deleteMany(ids, options = {}) {
        return this.#withWriteLock(() => this.#deleteMany(ids, options));
    }
    ...
    async #updateOne(docIdentifier, updateData = null, contextSpec = null, featureBitmapArray = []) {
        ...
        try {
            await this.#withDeferredMembership(async () => {
                ...
                await this.#removeDocumentTimelines(updatedDocument.id, previous, updatedDocument);
                ...
                this.#syncDocumentRelations(updatedDocument.id, previous.relations, documentRelations(updatedDocument));
                ...
                await this.#removeStaleLocationMembership(updatedDocument.id, { locations: previous.locations, orphanedAt: previous.orphanedAt }, updatedDocument, featureBitmaps);
    ...
    #emitEvent(...args) {
        const context = this.#writeContext.getStore();
        if (context?.active) { context.events.push(() => super.emit(...args)); return false; }
        return super.emit(...args);
    }

    // Trees use this bridge so their own listeners, as well as facade listeners,
    // only observe events from committed document writes.
    ...
    #emitMembershipChanged(changes) {
        ...
        this.#emitEvent(EVENTS.MEMBERSHIP_CHANGED, createEvent(EVENTS.MEMBERSHIP_CHANGED, { changes }));
    }
