...
- **Primary interval — dual-BSI.** The first entry (or the one flagged `primary: true`) is the document's canonical sortable value: `sortBy` uses it, and only it, so sort semantics stay unambiguous. **Open-ended** intervals are unrestricted: an open primary lives in the dual-BSI via its sentinel extremes, and open *non-primary* entries land in the membership plane's **open-interval sidecar** — per-scale dual-BSI tiers (`internal/tsm/<tl>/open/<scale>/{start,end}`) holding, per document, the min start of its ongoing (`[s,+∞)`) entries and the max end of its since-forever (`(-∞,e]`) entries. One value per (doc, scale, side) is lossless for membership because open intervals absorb (`[s1,+∞) ∪ [s2,+∞) = [min(s1,s2),+∞)`); the row keeps every entry and ref verbatim, only the index collapses. So a document can carry several ongoing facts on one timeline ("still in the Cenozoic" + "person still alive", both distilled into `wikipedia`) — the entry grammar is the only surface, plane routing is the engine's business.
- **Every other entry — tiled membership plane** (`internal/tsm/<timeline>/<scale>/{c,a}/<cellId>`). At ingest the interval is decomposed into its minimal hierarchical covering at the entry's own **floor** (coarse cells mid-span, floor cells at the boundaries — the FeatureBase `viewsByTimeRange` walk, generalized over our scale tiers and used at ingest as well as query time); covering cells are ticked in the cover plane, their coarser containers in the ancestor plane. Range queries decompose the same way and union the matching cells. Cell bitmaps hold doc ids, so timeline positions AND natively with context/feature/geo filters.

**Adaptive floor.** The finest cell scale of each covering comes from the entry's own notation — `'1769'` tiles at year cells, `'1769-08-15'` at day cells, `'541 MYA'` at Myr cells — with an explicit `scale` as override; sub-day notations clamp to `day` until an hour/minute tier exists. There is **no per-timeline granularity config**: one timeline carries geological eras, lifespans and single events at once, each at its own precision, and rebuilds derive everything from rows alone. Queries decompose at *their* notation-derived floor symmetrically, and the cover/ancestor two-plane probe glues mixed floors soundly (finer-stored docs surface via ancestor ticks, coarser-stored ones via cover ticks on the query cells' ancestors). Precision follows the notation on both sides — you get out the precision you put in; no row refinement in v1 (Pilosa precedent; an `exact` opt-in can land later without a format change). `scripts/bench-timeline-quantum.js` measures the cost: on a Wikipedia-shaped mix, an all-day-notation corpus costs ~2.3× the cells of all-year, while the realistic mixed-notation corpus sits in between with the best insert throughput — day cost is only paid where the data is day-precise.
...
- **Series in the supported RRULE subset** (`FREQ=DAILY|WEEKLY|MONTHLY|YEARLY`, `INTERVAL`, `COUNT`, `UNTIL`, `BYDAY` for weekly — the shapes calendar sync actually emits) are **expanded into per-occurrence entries**: the first occurrence is the primary sortable interval, the rest are membership positions. A weekly standup answers "what happens this week" only in weeks that actually contain an occurrence, at day tiles (occurrence timestamps are sub-day, so their adaptive floor clamps to `day`). Invalid dates are skipped per RFC 5545 (Jan 31 monthly skips February; Feb 29 yearly skips non-leap years). **Unbounded and over-cap rules are hybrid**: the first 512 occurrences expand exactly, and the un-expanded remainder becomes one never-miss tail entry from the next occurrence's start — open (living in the membership plane's open-interval sidecar) when the rule never ends, bounded to `UNTIL` otherwise. The standing weekly with no end date is per-occurrence exact for its first ~10 years and a superset beyond.
